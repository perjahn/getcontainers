...
.nodiff { color: #00FF00; }
    ...
    static async Task<List<Pod>> GetAllPods(string[] includeClusters, string[] excludeClusters, int timeoutSeconds)
    {
        ...
        var rows = await File.ReadAllLinesAsync(filename);
        List<string> clusters = [];

        var config = ConfigReader.Parse(rows);
        ...
        {
            Console.WriteLine($"Connecting to: {config.Clusters.Single(c => c.Name == cluster).Server} ({cluster})");
            using HttpClient client = new() { BaseAddress = new(config.Clusters.Single(c => c.Name == cluster).Server) };

            client.DefaultRequestHeaders.Add("Authorization", $"Bearer {config.Users.Single(u => u.Name == config.Contexts.Single(c => c.Name == cluster).User).Token}");
